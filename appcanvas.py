...
# These come from the repo (MUST be after sys.path.append)
...
# NOW add your constants and functions
...
def dump_course_smart(course_id: int) -> dict:
    ...
    modules = _get(f"{CANVAS}courses/{course_id}/modules", include=["items"])
...
# ALL YOUR ENDPOINTS HERE
@app.get("/health")
def health():
    """Health check - verifies Canvas API connection."""
    try:
        me = _get(f"{CANVAS}users/self/profile")
        return {"ok": True, "user": me.get("name"), "domain": CANVAS, "auth_required": bool(API_KEY)}
    except Exception as e:
        return {"ok": False, "error": str(e)}

@app.get("/dump")
def dump_get_blocking(course_id: int = Query(..., gt=0), auth=require_api_key):
    """Blocking dump - waits until finished."""
    try:
...
@app.get("/dump/async")
def dump_get_async(course_id: int = Query(..., gt=0), background_tasks: BackgroundTasks = None, auth=require_api_key):
    ...
    return {"accepted": True, "job_id": job_id, "course_id": course_id, "message": f"Job started. Check status at GET /status/{job_id}"}

@app.post("/dump/async")
def dump_post_async(body: DumpBody, background_tasks: BackgroundTasks, auth=require_api_key):
    """Non-blocking dump via POST."""
    return dump_get_async(body.course_id, background_tasks)

@app.get("/dump/smart")
def dump_smart(course_id: int = Query(..., gt=0), auth=require_api_key):
    """Smart sync - extracts text from PDFs/slides, skips videos."""
    try:
